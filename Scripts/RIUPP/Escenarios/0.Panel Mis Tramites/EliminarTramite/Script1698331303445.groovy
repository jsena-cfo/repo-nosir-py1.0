...
def datos = WebUI.callTestCase(findTestCase('RIUPP/Genericos/0.Queries/ObtenerProveedorConTipoTramite'), [('tipoTramite') : 1], FailureHandling.STOP_ON_FAILURE)
...
WebUI.click(findTestObject('BAC/Menu Mis Tramites/btnAccionesTramite', [('tramite') : nroTramiteQuery]))

WebUI.click(findTestObject('BAC/Menu Mis Tramites/liAcciones', [('accion') : 'Eliminar Tramite']))

WebUI.verifyTextPresent('¿Está seguro de eliminar el trámite?.', false)

WebUI.click(findTestObject('BAC/Menu Mis Tramites/btnEliminar'))

WebUI.clearText(findTestObject('BAC/Menu Mis Tramites/inputNumeroTramite'))
...
if (tramitesProveedor.size() != 0) {
    ...
    for (int i = 1; i <= tramitesProveedor.size(); ++i) {
        ...
        assert nroTramite != nroTramiteQuery : 'El tramite se visualiza en pantalla, no se pudo eliminar'
    }
}
