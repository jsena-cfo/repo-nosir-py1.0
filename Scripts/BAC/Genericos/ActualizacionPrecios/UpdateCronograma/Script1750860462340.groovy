...
String hoy= CustomKeywords.'tool.DateGenerator.todayFormat'("yyyy-MM-dd")

String ayer= CustomKeywords.'tool.DateGenerator.yerterday'("yyyy-MM-dd")

//ayer = CustomKeywords.'tool.DateGenerator.yerterday'('aaaa-MM-dd')

String dentroDeVariosDias= CustomKeywords.'tool.DateGenerator.dateNDaysLaterToday'(2, "yyyy-MM-dd")

System.out.println('Ayer: ' + ayer)

System.out.println('Fecha despues de 7 dias: ' + dentroDeVariosDias)
...
//u53r534c09    dTUzcjUzNGMwOQ==
//connection = CustomKeywords.'com.katalon.plugin.keyword.connection.DatabaseKeywords.createConnection'(DBType.sqlserver,
//	(mapDB['varServer']), mapDB[('varPort')], mapDB[('varDBName')], mapDB[('varUser')], mapDB[('varPass')])

//connection = CustomKeywords.'com.katalon.plugin.keyword.connection.DatabaseKeywords.createConnection'(DBType.sqlserver,
//	'10.17.24.30', '1436', 'BAC_TEST', 'usrSEAC', 'dTUzcjUzNGMwOQ==')
...
//sql = CustomKeywords.'tool.SqlQueriesBac.queryUpdateFechasCronograma'(numeroOca, ayer, dentroDeVariosDias)

sql = CustomKeywords.'tool.SqlQueriesBac.queryUpdateFechasCronograma'(numeroOca, hoy, ayer, dentroDeVariosDias)
...
CustomKeywords.'tool.sqlServerDB.ejecutarUpdate'(sql)

//CustomKeywords.'tool.sqlServerDB.ejecutarQuery'(sql)
