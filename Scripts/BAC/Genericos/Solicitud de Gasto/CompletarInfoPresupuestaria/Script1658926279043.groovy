...
for (int i = 1; i <= cantElementos.size(); ++i) {
    ...
    if (comuna == '') {
        ...
        WebUI.setText(findTestObject('BAC/Solicitud de Gasto/Page_BAC - Completar info presupuestaria/inputMontoPartida', [('comuna') : comuna]), 
            monto)

        WebUI.check(findTestObject('BAC/Solicitud de Gasto/Page_BAC - Completar info presupuestaria/checkSeleccionarPartida', 
                [('comuna') : comuna]))
    }
