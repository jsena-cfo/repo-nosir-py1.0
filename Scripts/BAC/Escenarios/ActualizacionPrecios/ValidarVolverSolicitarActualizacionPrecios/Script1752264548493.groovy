...
/**Author: Jer**/
/**Script valida el funcionamiento del boton Volver**/

boolean volverValidado = false

mapBoolean = [('volverExiste') : false, ('volverFunciona') : false]
...
WebUI.callTestCase(findTestCase('BAC/Genericos/ActualizacionPrecios/UpdateCronograma'), [('varUrlVar') : urlVar, ('varNumeroOca') : numeroOCA], 
...
(mapBoolean['volverExiste']) = WebUI.verifyElementPresent(findTestObject('BAC/ActualizacionPrecios/Page_BAC - Volver/a_Volver'), 
    2)

WebUI.callTestCase(findTestCase('BAC/Genericos/ActualizacionPrecios/ClickVolver'), [:], FailureHandling.STOP_ON_FAILURE)

(mapBoolean['volverFunciona']) = WebUI.verifyElementPresent(findTestObject('BAC/ActualizacionPrecios/Page_BAC - Volver/h3_Mi escritorio'), 
    2)

System.out.println('MapBoolean: ' + mapBoolean)

volverValidado = !(mapBoolean.containsValue(false))

WebUI.callTestCase(findTestCase('BAC/Genericos/ActualizacionPrecios/Logout'), [:], FailureHandling.STOP_ON_FAILURE)

System.out.println('Numero OCA: ' + numeroOCA)

CustomKeywords.'tool.Logger.log'('Numero OCA: ' + numeroOCA)

assert volverValidado
