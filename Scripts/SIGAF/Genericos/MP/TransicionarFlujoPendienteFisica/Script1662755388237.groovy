...
String usuarioVar = getBinding().getVariables().getAt('varUsuario').toString()
String passwordVar = getBinding().getVariables().getAt('varPassword').toString()
String numeroVar = getBinding().getVariables().getAt('varNumero').toString()
String ejercicioVar = getBinding().getVariables().getAt('varEjercicio').toString()
...
WebUI.callTestCase(findTestCase('SIGAF/Genericos/MP/TransicionarPendienteFisica'), [:], FailureHandling.STOP_ON_FAILURE)
...
System.out.println("Estado actualizado aux2: " + estadoActualizadoAux)

WebUI.callTestCase(findTestCase('SIGAF/Genericos/Varios/Salir'), [:], FailureHandling.STOP_ON_FAILURE)

WebUI.callTestCase(findTestCase('SIGAF/Genericos/Varios/Logout'), [:], FailureHandling.STOP_ON_FAILURE)

if (estadoActualizadoAux == "PENDIENTE FISICA") {
    System.out.println('El estado cambio a PENDIENTE FISICA')

    assert true
} else {
    System.out.println('El estado no cambio a PENDIENTE FISICA')
