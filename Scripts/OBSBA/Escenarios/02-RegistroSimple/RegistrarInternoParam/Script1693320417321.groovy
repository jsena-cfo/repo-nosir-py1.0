...
String fileXlsUsuarios = getBinding().getAt('varXlsInternos').toString()

String password = getBinding().getAt('varPassword').toString()

String url = getBinding().getAt('varUrl').toString()

Integer paramRetorno = getBinding().getAt('varParamRetorno')

//String dataFile= 'UsuariosSigaf'
//String dataFile = 'obsbaInternosAlt'
String dataFile = getBinding().getAt('varDataFileInternos').toString()

//String path = '\\Xls\\obsba\\usersSigafXls\\usuariosInternos.xlsx'
//String path = '\\Xls\\obsba\\usersSigafXls\\' + fileXls
  ...
  for (int i = 1; i <= row_control; i++) {
    ...
    if (estado == '') {
        ...
        if (registroSatisfactorio == true) {
            ...
            if (paramRetorno.equals(1)) {
                //retorna el cuit
                return cuitDisponible.toString() //retorna el cuit y el usuario
    ...
    if (index > row_control) {
		...
		if((mapData['1']) != usuarioVal) {
            String mensajeBD = 'Ocurrio un error. No se encontro usuario!'
...
if (((mapData['1']) != (null)) && ((mapData['2']) != (null))) {
   ...
   if (registroSatisfactorio == true) {
	  ...
	  WebUI.callTestCase(findTestCase('OBSBA/Genericos/ActivarUsuario'), [('varCuil') : cuit, ('varToken') : token],
		FailureHandling.STOP_ON_FAILURE)
	  ...
	  if (paramRetorno.equals(1)) {
		...
		def mapDatos = [('cuil') : cuit, ('usuario') : usuario]
