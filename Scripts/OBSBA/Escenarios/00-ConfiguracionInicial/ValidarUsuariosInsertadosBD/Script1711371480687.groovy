...
//String sql2= 'SELECT * FROM slu.busuario a WHERE a.c_user LIKE' + "'" + '%REGRE%' + "'" + 'AND a.xc_user LIKE ' + "'" + '%REGRESION%' + "'"

String sql= 'SELECT x.*, rownum FROM (SELECT a.c_user, a.xc_user, a.fh_alta, a.n_documento FROM slu.busuario a WHERE a.c_user LIKE ' + "'" + '%REGRE%' + "'" +  ' AND a.xc_user LIKE ' + "'" + '%REGRESION%' + "'" +  ' ORDER BY a.n_documento DESC) x WHERE rownum = 1'
...
if(vacio== true) {
	System.out.println("No hay usuario insertados")
	numeroDni= "00000000"
}
else {
	numeroDni= CustomKeywords.'com.katalon.plugin.keyword.connection.ResultSetKeywords.getListCellValue'(actorData, 'N_DOCUMENTO')
	println('Numero Documento: ' + numeroDni)
